...
- **Vertical landing owner (overlay):** after layout/DOM probe picks target **wire**, resolve focus doc pos via `paintContextForWire(wire).paintDocPos` when measured, else `docPosAtContentWire` — same owner as click ingress and strand read.
...
When the same wire offset aliases multiple doc owners (mention-start vs cross-row sandwich spacer tail, probe vs content-row end), **paint, layout row, and caretKind follow focus doc position → paint doc position → measured geometry** — not raw wire alone.

- **Cross-row sandwich spacer:** mention-start at a wire shared with a spacer text tail maps to the spacer tail **only** when spacer and following pill are on **different visual rows** (midY gap > 4px). Adjacent pills on the same row keep separate paint owners.
- **`layoutRowForFocus` / `layoutRowIndexForDocPos`:** canonical row lookup from focus doc pos via paint path and `paintContextForWire`. Mention focus uses mention doc pos directly (pill row). Text focus uses paint doc pos.
- **`describeCaretContext(doc, focus, { root? })`:** caretKind from paint doc pos wire — never from alias wire when focus is spacer text tail.
- **`paintContextForWire`:** wire-keyed index built at layout acquire; maps alias wires to `{ paintDocPos, rowIndex, top }`.
...
- **Cross-row sandwich spacer is not soft-wrap continuation:** whitespace-only pre-mention gap painted on row N with following pill on row N+1 is **not** `isWrapContinuationStartClickInRowAbove` / `viewportAliasesStaleWrapContinuation`. Click ingress uses `layoutRowForFocus`, not `rowIndexForWire(liveWire)`.
...
- **DOM boundary (read/write):** `setDocSelection` writes via paint-aware DOM points. Passive strand read applies authority doc pos on same-wire alias seams. **Click ingress** resolves focus through layout `paintContextForWire` / paint doc pos before returning — wire alias owners follow measured row geometry, not raw DOM node kind alone.
...
- **DOM boundary mapping:** `domPointToDocPos` applies click ingress Rule 4 at ingress — text-node tails belong to content row end; blank-band probes come only from blank infrastructure DOM. **Paint at probe aliases** (`resolveDomPointAtDocPos`, `domPointAfterWireBreak`) passes **focus doc position** into `embeddedBlankBandAtEmptyContentRowEnd` so cleared content row end paints wire-break `<br>`, not blank-band anchor. **`resolvePaintDocPos`** applies cross-row sandwich spacer alias for DOM paint only. **`isEmbeddedBlankBandDeleteProbeWire(doc, wire, focus)`** requires focus when the wire aliases cleared row end — wire-only checks misclassify spacer-chip landing. Delete policy uses the blank-band collapse family in **Backspace / Delete at blank-band probes**.
