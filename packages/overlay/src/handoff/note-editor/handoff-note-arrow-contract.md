...
- **Resolution order (overlay):**
  ...
  - **Fallback** — when strict live and viewport pass both miss, accept browser live at content wire — resolution **`nativeSelection`** (includes wrap-continuation live on the row below when the click landed past the upper row’s content extent). No layout-inferred row-end repair on click; `layoutContentRowEndWire` / sticky column remain for **vertical arrow** stickiness only.
