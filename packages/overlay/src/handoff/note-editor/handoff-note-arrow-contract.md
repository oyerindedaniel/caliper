...
- **Wire newline line box:** wire `\n` paints as a break. When the document ends with `\n`, a **non-wire** DOM line-pad `<br>` opens the **trailing empty line** and owns that stop’s paint. The final wire `\n` before that pad is a blank probe that paints the **bare wire-break** — do **not** emit a blank-anchor immediately before the pad (those two docks share one band in the browser). The trailing empty line’s navigable stop is its **line-start** (document end when the final segment is empty). Do **not** collapse that stop onto the previous blank’s break probe, and do **not** invent a separate **content** row at document end. Up/Down step every empty line slot in the lattice.
- **One probe → one dock:** for each blank-band probe, dock kind is decided once (bare wire-break vs blank-anchor ZWSP). Emit only that dock; paint only that dock. Never choose bare paint while a blank-anchor sibling exists for the same break — the browser may park the caret on the sibling and leave memory on the probe wire (wrong visual row). When a blank-anchor is emitted, it follows its wire-break; when the probe docks bare, omit the blank-anchor. Line-start anchor always precedes its row-start atom — orphan docks are corrupt and must not be remapped to document end or document start.
...
- **Blank navigable stops = empty line slots** at each empty line’s **line-start wire** (including document end when the wire ends in `\n`). Line count is newlines + 1; blank-only `"\n\n\n\n"` → five stops `0..4`. Content-bearing `"header\n\n"` → empty line-starts only (not the content row’s terminating `\n`). Probe wires remain delete/paint docks and must not be a second caret identity for the same empty line — no probe↔length alias glue.
...
- **Empty line slot** → **blank visual row** with a navigable stop at that line’s start wire (document end when the final segment is empty). Do not collapse consecutive empty lines into one. Do not invent an extra **content** row for that trailing empty line. Horizontal Right (and vertical Down bleed) from the last empty line does not invent a further stop beyond it.
...
- **Paint vs intent:** same wire may alias probe infrastructure and content-row end. **Intent** uses structure + focus. **Paint** for breaks is always the break path, and **emit matches that dock** (one probe → one dock): bare `<br>` — and no blank-anchor sibling — when the empty content-row end has an empty row above (emptied chip / prefix-only / cleared interior — first visual line), when sole-char content still abutting, or when the probe is the final wire `\n` before EOF line-pad; blank-anchor ZWSP when delete-probe owns the blank visual row, when empty content-row end has multi-char substantive abutting (Shift+Enter blank-row dock under content), or when empty content-row end has only trailing whitespace above (except the pad-preceding probe, which stays bare). Content row end with text still on the row focuses the **last character** — do not remap the break wire itself to a text-tail.
...
### Directional nibbling

- **In-row text:** backspace behind; delete ahead; caret stays at the deletion point. Sole/last content char before `\n` (including leftover commit space) keeps `before` so the next insert/paint stay at visual start — not content-row-end text-tail. At visual start of a populated row, backspace does not nip that row’s text; at content row end, delete does not nip further on that row without moving.
- **Progressive trash (blanks and band edges only):** land so the **next** key continues in that direction — backspace → blank above or content-row-end above with `after`; when collapsing a blank **under content** (trailing-only or sandwiched), remount `after` on that content char (next key chips) — do not park omit on a leftover blank; delete → next blank below or visual start on content below. Not after ordinary in-row nibble (`before` stays).
- On **blank** rows, each key nips **that blank** only.
...
- **Blank-band collapse (Backspace):** one blank-row `\n` per key; land **upward**. Under content (sandwiched or trailing-only) remount content-row-end with `after` on that collapse — leftover empties toward a lower filled row stay until focused as blank rows. Leading blanks with no content above → continue one blank at a time (EOF / last blank stop must not jump to band head while a higher empty line-start remains); when the band is exhausted toward content, land next row visual start below. Leading blank under content (band head) is empty content row end for click/chip.
...
- **Blank-band collapse (Delete):** mirror **downward** one blank at a time — next blank below when one remains, else lower content visual start. Do not jump to band head (blanks above). Do not skip remaining blanks below in one stroke. Never land content-row-end above on Delete. Blank visual line-start stops that are not probe docks use the same stop→opener collapse entry as Backspace (probe-coincident stops stay on the probe / empty-CRE path). Delete **land** after that collapse is owned by the prior stop (map stop into the next wire, then next blank at/after that floor above content, else content). A leftover leading blank with nothing substantive above is not a downward park — continue to content. Mid-doc remaining empties under content stay. Do not reuse the opener probe’s land as Delete authority.
- **Band edge no-op** only when nothing remains to merge in that direction and no substantive row to land on — e.g. last trailing blank under content (`TOP\n`). Sole leftover prefix blank with no substantive content clears to empty on Delete (same end state as Backspace).
...
- **Blank rows (horizontal):** same rule as vertical — one empty segment between breaks is one blank hop. Enter blank from the row above in one Right; leave blank to the next content visual start in one Right. Do **not** stop on the non-probe `\n` that only starts the lower content row. Left mirrors.
