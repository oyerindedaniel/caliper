...
- **Blank-band collapse (Delete):** mirror **downward**; last blank before content below → lower visual start. Never land content-row-end above on Delete. Blank visual line-start stops that are not probe docks use the same stop→opener collapse entry as Backspace (probe-coincident stops stay on the probe / empty-CRE path).
