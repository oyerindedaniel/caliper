...
export function toPixels(
  ...
  if (trimmed.startsWith("var(--") && trimmed.endsWith(")") && options.tokens) {
    ...
    const resolved =
      options.tokens.spacing[tokenName] ||
      options.tokens.borderRadius[tokenName] ||
      options.tokens.colors[tokenName] ||
      (options.tokens.typography[tokenName]
        ? `${options.tokens.typography[tokenName]!.fontSize}px`
        : undefined);
  ...
  if (trimmed.endsWith("vmin"))
    return (num / 100) * Math.min(context.viewportWidth, context.viewportHeight);
  if (trimmed.endsWith("vmax"))
    return (num / 100) * Math.max(context.viewportWidth, context.viewportHeight);

  if (trimmed.endsWith("svw") || trimmed.endsWith("lvw"))
    return (num / 100) * context.viewportWidth;
  if (trimmed.endsWith("svh") || trimmed.endsWith("lvh"))
    return (num / 100) * context.viewportHeight;

  // Dynamic Viewport Units (Use visualViewport if available)
  if (trimmed.endsWith("dvw"))
    return (num / 100) * (context.visualViewportWidth ?? context.viewportWidth);
  if (trimmed.endsWith("dvh"))
    return (num / 100) * (context.visualViewportHeight ?? context.viewportHeight);

  // Container Query Units (Fallback to small viewport units as per spec if no container)
  if (trimmed.endsWith("cqw") || trimmed.endsWith("cqi"))
    return (num / 100) * context.viewportWidth;
  if (trimmed.endsWith("cqh") || trimmed.endsWith("cqb"))
    return (num / 100) * context.viewportHeight;
  if (trimmed.endsWith("cqmin"))
    return (num / 100) * Math.min(context.viewportWidth, context.viewportHeight);
  if (trimmed.endsWith("cqmax"))
    return (num / 100) * Math.max(context.viewportWidth, context.viewportHeight);
...
export function evaluateResolvedMath(
  expression: string,
  context: ContextMetrics,
  options: CalcOptions
): number {
