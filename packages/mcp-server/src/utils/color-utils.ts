...
function parseLab(lab: string): NormalizedColor {
    ...
    const Z65 = X50 * 0.0122982 + Y50 * -0.0204830 + Z50 * 1.3299098;
...
function xyzToOklab(x: number, y: number, z: number, alpha: number, raw: string): NormalizedColor {
    ...
    const sValue = 0.0482003018 * x + 0.2643662691 * y + 0.6338517070 * z;
    ...
    return {
        l: 0.2104542553 * lCubeRoot + 0.7936177850 * mCubeRoot - 0.0040720403 * sCubeRoot,
...
function rgbToNormalized(red: number, green: number, blue: number, alpha: number, raw: string): NormalizedColor {
    ...
    const y = 0.2126729 * linearRed + 0.7151522 * linearGreen + 0.0721750 * linearBlue;
    const z = 0.0193339 * linearRed + 0.1191920 * linearGreen + 0.9503041 * linearBlue;

    return xyzToOklab(x, y, z, alpha, raw);
}

function p3ToNormalized(red: number, green: number, blue: number, alpha: number, raw: string): NormalizedColor {
    ...
    const z = 0.0000000 * linearRed + 0.0451104 * linearGreen + 1.0439444 * linearBlue;
