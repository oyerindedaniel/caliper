# Changelog

All notable changes to this project will be documented in this file.

## [0.1.0] - 2026-01-26

### Added
- Initial release of `@oyerinde/caliper-mcp`.
- **MCP tools**: `caliper_list_tabs`, `caliper_switch_tab`, `caliper_inspect`, `caliper_measure`, `caliper_get_state`, `caliper_clear`, `caliper_grep`, `caliper_walk_dom`, `caliper_parse_selector`, `caliper_walk_and_measure`, and `caliper_semantic_reconcile`.
- **MCP prompts**: `caliper-selector-audit`, `caliper-audit-harness`, `caliper-figma-reconcile`, and `caliper-cross-compare`.
- **MCP resources**: `caliper://state` and `caliper://tabs`.
- CLI support for dynamic port assignment via `--port/-p`.
- Zero-dependency runtime.
