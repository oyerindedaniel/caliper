...
export class NavigationSession {
  ...
  async scrollTo(scrollX?: number, scrollY?: number): Promise<{ scrollX: number; scrollY: number }> {
    const targetScrollX =
      scrollX === undefined ? "window.scrollX" : Number.isFinite(scrollX) ? String(scrollX) : "window.scrollX";
    const targetScrollY =
      scrollY === undefined ? "window.scrollY" : Number.isFinite(scrollY) ? String(scrollY) : "window.scrollY";
